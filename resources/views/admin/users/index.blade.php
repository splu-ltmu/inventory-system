        ...
        <tbody>
            @forelse($users as $user)
                <tr style="border-bottom:1px solid #e0e7ff; background:linear-gradient(135deg, #ffffff, #f8fafc);">
                    ...
                    <td style="padding:14px 10px; border-bottom:1px solid #e0e7ff; color:#64748b; font-size:14px;">{{ $user->email }}</td>
                    <td style="padding:14px 10px; border-bottom:1px solid #e0e7ff; color:#64748b; font-size:14px;">{{ $user->office ?? '-' }}</td>
                    <td style="padding:14px 10px; border-bottom:1px solid #e0e7ff; color:#475569; font-weight:600;">{{ $user->role ?? '-' }}</td>
                    <td style="padding:14px 10px; border-bottom:1px solid #e0e7ff; color:#475569; font-weight:600;">{{ $user->created_at->format('M d, Y') }}</td>
                    <td style="padding:14px 10px; border-bottom:1px solid #e0e7ff;">
                        <button type="button" onclick="openEditModal({{ $user->id }}, '{{ $user->name }}', '{{ $user->email }}', '{{ $user->office ?? '' }}', '{{ $user->role ?? '' }}')" class="btn-action btn-edit" style="padding:8px 16px; border-radius:8px; border:2px solid #3b82f6; background:linear-gradient(135deg, #3b82f6, #1d4ed8); color:#ffffff; font-weight:700; transition:all 0.3s ease; box-shadow:0 4px 12px rgba(59,130,246,0.2);">Edit</button>
