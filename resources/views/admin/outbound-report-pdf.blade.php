    ...
    <tbody>
        @forelse($outbounds as $row)
            <tr>
                ...
                <td>{{ $row->stock?->description ?? $row->description ?? '—' }}</td>
                <td>{{ $row->stock?->unit ?? '—' }}</td>
                <td class="tc">{{ $row->total ?? 0 }}</td>
                <td>{{ $row->office ?? '—' }}</td>
                <td>{{ $row->member?->name ?? ($row->client?->name ?? '—') }}</td>
                <td>{{ ucfirst($row->approval ?? 'pending') }}</td>
                <td>{{ ucfirst($row->status ?? 'released') }}</td>
                <td>{{ $row->received_by ?? '—' }}</td>
                <td>{{ \Carbon\Carbon::parse($row->deducted_at ?? $row->created_at)->format('M d, Y') }}</td>
            </tr>
        @empty
            <tr>
