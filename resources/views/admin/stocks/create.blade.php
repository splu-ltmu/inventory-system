...
<div class="toolbar">
    ...
    <a class="btn-link" href="{{ route('stocks.index') }}">Back to Stocks</a>
</div>
...
<div class="form-container">
    ...
    <form action="{{ route('stocks.store') }}" method="POST">
        ...
        <div class="form-group">
            ...
            <input type="text" name="id_no" id="id_no" value="{{ old('id_no') }}" required readonly style="background-color: #f5f5f5; cursor: not-allowed;">
        </div>
        ...
        <div class="form-checkbox">
            <input type="checkbox" name="hidden" id="hidden" value="1" {{ old('hidden') ? 'checked' : '' }}>
            <label for="hidden">Hidden (Admin Only)</label>
        </div>

        <div class="form-actions">
            <button type="submit" class="btn-submit">Add Stock</button>
            <a href="{{ route('stocks.index') }}" class="btn-cancel">Cancel</a>
        </div>
    </form>
</div>
