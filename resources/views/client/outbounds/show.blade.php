...
@section('content')
    <div style="margin-top:16px;">
        ...
        <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:20px;">
            <div>
                <h2 style="color:#1e40af; font-size:24px; font-weight:700; margin:0;">Outbound #{{ $outbound->id }}</h2>
                <p style="color:#64748b; font-size:14px; margin:4px 0 0 0;">Created on {{ $outbound->created_at->format('F j, Y, g:i A') }}</p>
            </div>
            ...
            <div style="padding:20px 24px; background:linear-gradient(135deg, #10b981, #059669); border-bottom:1px solid #e2e8f0;">
                <div style="display:flex; justify-content:space-between; align-items:center;">
                    <div>
                        ...
                        <div style="color:#fff; font-size:18px; font-weight:700; margin-top:2px;">{{ ucfirst($outbound->status) }}</div>
                    </div>
                    <div style="text-align:right;">
                        <div style="color:#fff; font-size:12px; font-weight:600; text-transform:uppercase; letter-spacing:0.5px;">Outbound ID</div>
                        <div style="color:#fff; font-size:18px; font-weight:700; margin-top:2px;">#{{ $outbound->id }}</div>
                    </div>
                </div>
            </div>
            ...
            <div style="padding:24px;">
                <div style="display:grid; grid-template-columns: repeat(auto-fit, minmax(250px, 1fr)); gap:20px;">
                    ...
                    <div style="padding:16px; background:#f8fafc; border-radius:12px; border:1px solid #e2e8f0;">
                        ...
                        <div style="font-weight:700; color:#1e40af; font-size:16px; margin-bottom:4px;">{{ $outbound->stock->id_no }}</div>
                        <div style="color:#475569; font-size:14px;">{{ $outbound->stock->description ?? $outbound->stock->name ?? 'Item' }}</div>
                    </div>
                    ...
                    <div style="padding:16px; background:#f8fafc; border-radius:12px; border:1px solid #e2e8f0;">
                        ...
                        <div style="font-weight:700; color:#1e40af; font-size:24px; margin-bottom:4px;">{{ $outbound->total }}</div>
                    ...
                    <div style="padding:16px; background:#f8fafc; border-radius:12px; border:1px solid #e2e8f0;">
                        ...
                        @if($outbound->member)
                            <div style="font-weight:700; color:#1e40af; font-size:16px; margin-bottom:4px;">{{ $outbound->member->name }}</div>
                            <div style="color:#475569; font-size:14px;">{{ $outbound->member->email }}</div>
                        @else
                            <div style="font-weight:700; color:#059669; font-size:16px; margin-bottom:4px;">Direct Distribution</div>
                            <div style="color:#475569; font-size:14px;">{{ $outbound->office }}</div>
                        @endif
                    </div>
                    ...
                    <div style="padding:16px; background:#f8fafc; border-radius:12px; border:1px solid #e2e8f0;">
                        ...
                        <div style="font-weight:700; color:#1e40af; font-size:16px; margin-bottom:4px;">{{ $outbound->office }}</div>
                ...
                @if($outbound->reason)
                    <div style="margin-top:20px; padding:16px; background:#fef3c7; border-radius:12px; border:1px solid #fbbf24;">
                        ...
                        <div style="color:#78350f; font-size:14px; line-height:1.5;">{{ $outbound->reason }}</div>
                    </div>
                @endif
                ...
                <div style="margin-top:20px; padding:16px; background:#f0fdf4; border-radius:12px; border:1px solid #86efac;">
                    ...
                    <div style="display:flex; flex-direction:column; gap:8px;">
                        <div style="display:flex; justify-content:space-between; align-items:center;">
                            ...
                            <div style="color:#1e40af; font-size:14px; font-weight:600;">{{ $outbound->created_at->format('M j, Y g:i A') }}</div>
                        </div>
                        @if($outbound->deducted_at)
                            <div style="display:flex; justify-content:space-between; align-items:center;">
                                <div style="color:#475569; font-size:14px;">Inventory Deducted:</div>
                                <div style="color:#059669; font-size:14px; font-weight:600;">{{ $outbound->deducted_at->format('M j, Y g:i A') }}</div>
                            </div>
                        @endif
                    </div>
                </div>
