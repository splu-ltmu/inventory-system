...
@section('content')
    <div style="margin-top:16px;">
        ...
        <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:20px;">
            ...
            <a href="{{ route('client.outbounds.create') }}" 
                        ...
                        @foreach($outbounds as $outbound)
                            <tr style="border-bottom:1px solid #e0e7ff; background:linear-gradient(135deg, #ffffff, #f8fafc);">
                                <td style="padding:14px 10px; border-bottom:1px solid #e0e7ff;">
                                    <div style="font-weight:700; color:#1e40af; font-size:14px;">#{{ $outbound->id }}</div>
                                </td>
                                <td style="padding:14px 10px; border-bottom:1px solid #e0e7ff;">
                                    <div style="font-weight:600; color:#1e40af; font-size:13px;">{{ $outbound->stock->id_no }}</div>
                                    <div style="color:#64748b; font-size:11px; margin-top:2px;">{{ $outbound->stock->description ?? $outbound->stock->name ?? 'Item' }}</div>
                                </td>
                                <td style="padding:14px 10px; border-bottom:1px solid #e0e7ff; color:#475569; font-weight:600;">{{ $outbound->total }}</td>
                                <td style="padding:14px 10px; border-bottom:1px solid #e0e7ff;">
                                    @if($outbound->member)
                                        <div style="font-weight:600; color:#1e40af; font-size:13px;">{{ $outbound->member->name }}</div>
                                        <div style="color:#64748b; font-size:11px;">{{ $outbound->member->email }}</div>
                                    @else
                                        <div style="font-weight:600; color:#059669; font-size:13px;">Direct Distribution</div>
                                        <div style="color:#64748b; font-size:11px;">{{ $outbound->office }}</div>
                                    @endif
                                </td>
                                <td style="padding:14px 10px; border-bottom:1px solid #e0e7ff;">
                                ...
                                <td style="padding:14px 10px; border-bottom:1px solid #e0e7ff; color:#64748b; font-size:12px;">
                                    {{ $outbound->created_at->format('M j, Y') }}
                                </td>
                                <td style="padding:14px 10px; border-bottom:1px solid #e0e7ff;">
                                    <a href="{{ route('client.outbounds.show', $outbound) }}" 
