...
<div class="account-container">
            ...
            <form method="POST" action="{{ route('client.account.subaccounts.members.store', $subaccount) }}">
                ...
                <div class="form-grid">
                    <div class="form-group-enhanced">
                        ...
                        <input type="text" id="member_name" name="name" required value="{{ old('name') }}" placeholder="Enter full name">
                        @error('name')
                    ...
                    <div class="form-group-enhanced">
                        ...
                        <input type="email" id="member_email" name="email" placeholder="Optional email address" value="{{ old('email') }}">
                        @error('email')
                            <span style="color:#dc2626;font-size:12px;display:block;margin-top:4px;">{{ $message }}</span>
                        @enderror
                    </div>
                    ...
                    @foreach($members as $member)
                        ...
                        <div class="distribution-card">
                            ...
                            <div class="stats-grid" style="grid-template-columns: repeat(2, 1fr);">
                                ...
                                <div class="stat-item">
                                    <span class="stat-value">{{ $totalDistributed }}</span>
                            ...
                            @if(!$member->distributions->isEmpty())
                                ...
                                @endphp
                                @foreach($groupedDistributions as $stockRequestItemId => $group)
                                    @php
                                    ...
                                    <div style="display: flex; justify-content: space-between; align-items: center; padding: 6px 0; border-bottom: 1px solid rgba(0,0,0,0.05);">
                                        <div>
                                            ...
                                            <div style="font-size: 11px; color: var(--muted); margin-top: 2px;">Current Qty: {{ $totalDistributedForItem }}</div>
                                        </div>
                                        <div style="display: flex; align-items: center; gap: 8px;">
                                            <form method="POST" action="{{ route('client.account.subaccounts.distributions.update', [$subaccount, $firstDistribution]) }}" style="display:flex; align-items:center; gap:6px; margin:0;">
                                                @csrf
                                                @method('PUT')
                                                <input type="hidden" name="tab" value="members">
                                                <input type="number" class="member-update-qty" data-current="{{ $totalDistributedForItem }}" name="updated_qty" value="" placeholder="New qty" min="0" max="{{ max(0, $totalDistributedForItem - 1) }}" style="width:80px; padding:4px 8px; border:1px solid var(--line); border-radius:6px; font-size:12px;">
                                            ...
                                            <span class="used-count" style="font-size:11px; color: var(--muted);">Used: {{ $usedQtyForItem }}</span>
                                        </div>
                                    </div>
                                @endforeach
                                    </div>
                                ...
                                <tr>
                                    <td>
                                        <div class="item-header" style="margin: 0; gap: 12px;">
                                            <div class="item-icon" style="width: 32px; height: 32px; font-size: 14px;">{{ $initials }}</div>
                                            <div class="item-info" style="text-align: left;">
                                                <div style="font-weight: 600; color: var(--text);">{{ $itemName }}</div>
                                                <div style="font-size: 12px; color: var(--muted);">₱{{ number_format($item->stock->price ?? 0, 2) }}</div>
                                            </div>
                                        </div>
                                    </td>
                                    <td style="text-align: center; font-weight: 600; color: #2563eb;">{{ $allocation->allocated_qty }}</td>
                                    <td style="text-align: center; font-weight: 600; color: #059669;">{{ $memberQty }}</td>
                                    <td style="text-align: center; font-weight: 600; color: #dc2626;">{{ $distributed }}</td>
                                    <td style="min-width: 120px;">
                                        <div style="display: flex; flex-direction: column; gap: 4px;">
                                            <div class="progress-bar" style="width: 100%; margin: 0;">
                                                <div class="progress-fill" style="width: {{ $progressPercent }}%"></div>
                                            </div>
                                            <div style="font-size: 11px; color: var(--muted); text-align: center;">{{ number_format($progressPercent, 1) }}% used</div>
                                        </div>
                                    </td>
                                    <td>
                                        <div class="quick-actions" style="margin: 0; flex-direction: column; gap: 6px;">
                                            <button class="btn-quick" onclick="selectItem({{ $item->id }}, '{{ addslashes($itemName) }}', {{ $remaining }})" {{ $remaining <= 0 ? 'disabled' : '' }} style="width: 100%; padding: 6px 8px; font-size: 11px;">
                                                Quick Distribute
                                            </button>
                                            <button class="btn-quick" onclick="viewDistributionHistory({{ $allocation->id }})" style="width: 100%; padding: 6px 8px; font-size: 11px;">
                                                View History
                                            </button>
                                        </div>
                                    </td>
    ...
    <div class="distribution-form" id="distributionForm" style="display: none;">
        ...
        <form method="POST" action="{{ route('client.account.subaccounts.distributions.store', $subaccount) }}" id="distributionFormElement">
            @csrf
            ...
            @else
                ...
                <div class="form-group-enhanced">
                    ...
                    <div class="member-selector">
                        @foreach($members as $member)
                            <div class="member-card" onclick="selectMember({{ $member->id }}, this)">
    ...
    @if(!$allocatedItems->isEmpty() && !$members->isEmpty())
