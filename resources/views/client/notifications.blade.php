        ...
        <div style="display:grid; gap:12px;">
            @foreach($notifications as $notification)
                <div class="notification-item {{ !$notification->read ? 'unread' : '' }}" 
                    ...
                    <div class="notification-icon" style="width:40px; height:40px; border-radius:50%; display:flex; align-items:center; justify-content:center; flex-shrink:0; background:{{ $notification->color === 'green' ? 'var(--green-light)' : ($notification->color === 'red' ? 'var(--red-light)' : ($notification->color === 'orange' ? 'var(--orange-light)' : ($notification->color === 'yellow' ? 'var(--yellow-light)' : 'var(--blue-light)'))) }}; color:{{ $notification->color === 'green' ? 'var(--green)' : ($notification->color === 'red' ? 'var(--red)' : ($notification->color === 'orange' ? 'var(--orange)' : ($notification->color === 'yellow' ? 'var(--yellow)' : 'var(--blue)'))) }};">
                    ...
                    <div style="flex:1; min-width:0;">
                        <div style="display:flex; justify-content:space-between; align-items:flex-start; margin-bottom:4px;">
                            <h4 style="margin:0; font-size:16px; color:var(--text);">{{ $notification->title }}</h4>
                            <span style="font-size:12px; color:var(--muted); white-space:nowrap;">{{ $notification->created_at->diffForHumans() }}</span>
                        </div>
                        <p style="margin:0 0 8px 0; color:var(--muted); font-size:14px; line-height:1.4;">{{ $notification->message }}</p>
                        @if($notification->action_url)
                            <a href="{{ $notification->action_url }}" style="display:inline-flex; align-items:center; gap:4px; color:var(--blue); text-decoration:none; font-size:14px; font-weight:500;">
                                View Details
