                    ...
                    <select id="deduct_distribution_select" name="distribution_id" required style="width:100%; padding:12px; border:1px solid var(--line); border-radius:10px; font-size:14px; background:#fff; transition:border-color .2s ease;">
                        ...
                        <?php $__currentLoopData = $clientMembers; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $member): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                            <?php $__currentLoopData = $member->distributions; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $distribution): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                ...
                                <option value="<?php echo e($distribution->id); ?>"><?php echo e($distribution->stockRequestItem->stock->description ?? $distribution->stockRequestItem->stock->name ?? 'Item'); ?> — Available: <?php echo e($availableQty); ?></option>
                                <?php endif; ?>
                            <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                        <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                    </select>
                            ...
                            <div class="report-card">
                                ...
                                <div class="report-card-body" style="padding:20px;">
                                    <div class="report-stat">
                                        ...
                                        <span class="stat-value" style="color:#1e40af; font-weight:700; font-size:18px;"><?php echo e($memberInventoryTotals['total_distributed']); ?></span>
                                    </div>
                                                                        <div class="report-stat">
                                        <span class="stat-label" style="color:#059669; font-weight:600;">Available Items:</span>
                                        <span class="stat-value text-success" style="background:linear-gradient(135deg, #ecfdf5, #d1fae5); color:#059669; font-weight:700; font-size:18px;"><?php echo e($memberInventoryTotals['total_available']); ?></span>
                                    </div>
                                </div>
                            </div>
                        </div>
                        ...
                        <tbody>
                            <?php $__currentLoopData = $memberReports; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $memberReport): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                ...
                                <tr style="border-bottom:1px solid #e0e7ff; background:linear-gradient(135deg, #ffffff, #f8fafc);">
                                    ...
                                    <?php echo e($memberReport['distributed_items']); ?>
    ...
    <div id="member-items-<?php echo e($memberReport['name']); ?>" 
        ...
        <div style="max-height:300px; overflow-y:auto; pointer-events:auto;" onclick="event.stopPropagation();">
            <?php
                ...
                // Add direct deductions
                if($currentMember && $currentMember->directDeductions->count() > 0) {
                    foreach($currentMember->directDeductions as $deduction) {
                        $itemName = $deduction->stockRequestItem->stock->description ?? 'Item';
                        
                        if($allItems->has($itemName)) {
                            // Update existing item
                            $existing = $allItems->get($itemName);
                            $existing->deducted_qty += $deduction->deducted_qty;
                            $existing->has_deducted = true;
                        } else {
                            // Add new item
                            $allItems->put($itemName, (object)[
                                'name' => $itemName,
                                ...
                                'used_qty' => 0,
                                'available_qty' => 0,
                                'deducted_qty' => $deduction->deducted_qty,
...
function openDeductModal(memberId, memberName, availableQty) {
    ...
    if (member && member.distributions) {
        member.distributions.forEach(distribution => {
            ...
            if (availableQty > 0) {
                ...
                option.textContent = `${distribution.stock_request_item?.stock?.description || distribution.stock_request_item?.stock?.name || 'Item'} — Available: ${availableQty}`;
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH /var/www/resources/views/client/account.blade.php ENDPATH**/ ?>